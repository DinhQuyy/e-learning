...
export async function PATCH(
  ...
  try {
    ...
    if (body.description !== undefined)
      quizPayload.description = body.description || null;
    if (body.passing_score !== undefined)
      quizPayload.passing_score = body.passing_score;
    if (body.time_limit !== undefined)
      quizPayload.time_limit = body.time_limit || null;
    if (body.max_attempts !== undefined)
      quizPayload.max_attempts = body.max_attempts;
    ...
    if (Object.keys(quizPayload).length > 0) {
      ...
      if (!quizRes.ok) {
        ...
        const message =
          errorData?.errors?.[0]?.message || "Không thể cập nhật quiz.";
        return NextResponse.json(
          { error: message },
    ...
    // If questions are provided, recreate them
    if (body.questions !== undefined) {
      // Delete existing questions (and their answers via cascade or manual)
      const existingQuestionsRes = await directusFetch(
        `/items/quiz_questions?filter[quiz_id][_eq]=${id}&fields=id,answers.id`
      );

      // If quizPayload was empty, this is the first call — check 401
      ...
      if (existingQuestionsRes.ok) {
        ...
        for (const question of existingQuestionsData.data ?? []) {
          ...
          // Delete question
          await directusFetch(
            `/items/quiz_questions/${question.id}`,
            {
              method: "DELETE",
            }
          );
    ...
    return NextResponse.json({ message: "Đã cập nhật quiz thành công." });
  } catch (error) {
...
export async function DELETE(
  ...
  try {
    ...
    // Delete answers and questions first
    ...
    if (!res.ok) {
      return NextResponse.json(
        { error: "Không thể xoá quiz." },
        { status: res.status }
      );
    }

    return NextResponse.json({ message: "Đã xoá quiz thành công." });
  } catch (error) {
