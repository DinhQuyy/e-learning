...
export default function EditQuizPage() {
  ...
  const loadQuiz = useCallback(async () => {
    try {
      const res = await apiFetch(`/api/instructor/quizzes/${quizId}`);
      if (!res.ok) throw new Error("Failed to load");
      const data = await res.json();
      const quiz = data.data;

      const lessonId =
        typeof quiz.lesson_id === "object" && quiz.lesson_id
          ? String(quiz.lesson_id.id ?? quiz.lesson_id)
          : quiz.lesson_id
      ...
      const questions = (quiz.questions ?? [])
        ...
        .map((q: { question_text: string; question_type: string; explanation: string | null; points: number; answers: { answer_text: string; is_correct: boolean; sort: number }[] }) => ({
          ...
          answers: (q.answers ?? [])
            .sort((a: { sort: number }, b: { sort: number }) => (a.sort ?? 0) - (b.sort ?? 0))
            .map((a: { answer_text: string; is_correct: boolean }) => ({
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      const res = await apiPatch(`/api/instructor/quizzes/${quizId}`, payload);

      if (!res.ok) {
        const errorData = await res.json().catch(() => null);
        throw new Error(errorData?.error || "Không thể cập nhật quiz");
      }
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground">
            Cập nhật câu hỏi và đáp án cho quiz
          </p>
        </div>
      </div>
