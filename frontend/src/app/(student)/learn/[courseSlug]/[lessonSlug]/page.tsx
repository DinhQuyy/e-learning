...
import { LessonStudyAssistant } from "@/components/features/lesson-study-assistant";
import { LessonAssignmentBlock } from "@/components/features/lesson-assignment-block";
...
import { buildLessonAiFaq } from "@/lib/ai-faq";
import { requireAuth } from "@/lib/dal";
import { directusUrl } from "@/lib/directus";
import { getCourseProgress, getEnrollmentByCourseSlug } from "@/lib/queries/enrollments";
...
export default async function LessonPage({
  ...
  if (!enrollment) {
    ...
    if (currentModule?.id) query.set("module", String(currentModule.id));
    query.set("lesson", String(lesson.id));
    redirect(`/courses/${courseSlug}?${query.toString()}#lesson-${lesson.id}`);
  }
  ...
  const allCompleted =
    allLessons.length > 0 && allLessons.every((item: Lesson) => completedLessonIds.includes(item.id));
  ...
  const currentPath = `/learn/${courseSlug}/${lessonSlug}`;
  const lessonFaq = buildLessonAiFaq(lesson.title, course.title);
    ...
    <div className="mx-auto w-full max-w-[1600px] px-4 py-6 lg:px-8 lg:py-8">
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_380px]">
        <div className="min-w-0 space-y-5">
          ...
          <AiFaqBlock config={lessonFaq} />

          <LessonAssignmentBlock lessonId={lesson.id} />
            ...
            <Card className="overflow-hidden rounded-2xl border-slate-200 bg-white shadow-sm">
              ...
              <CardContent className="p-5 sm:p-6">
                <QuizPlayer
                  ...
                  context={{ courseId: course.id, lessonId: lesson.id, currentPath }}
                />
              </CardContent>
            </Card>
          ) : null}
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              {prevLesson ? (
                ...
                <Button
                  variant="outline"
                  disabled
                  className="w-full justify-start rounded-xl border-slate-200 text-slate-300"
                >
